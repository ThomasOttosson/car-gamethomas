...
def check_ok(boat, taken):
    ...
    for i in range(len(boat)):
        ...
        if num in taken:  # Boat invalid if position is taken
            ...
            boat = [-1]
            break
        elif num % 10 == 9 and i < len(boat)-1:  # Stops wrapping to next row
            if boat[i+1] % 10 == 0:
                boat = [-1]
                break
        if i != 0:
            if boat[i] != boat[i-1]+1 and boat[i] != boat[i-1]+10:  # Rule
                boat = [-1]
                break
...
def calc_tactics(shot, tactics, guesses, hit):
    ...
    temp = []
    if len(tactics) < 1:
        temp = [shot-1, shot+1, shot-10, shot+10]  # Explore all adjacent cells
    else:
        if shot-1 in hit:
            temp = [shot+1]
...
for i in range(80):
    ...
    # Track all previous guesses made by player 1 hits, misses, completed ships
    guesses1 = hit1 + miss1 + comp1

# Prompt player 1 to take a shot at player 2's board
    shot1 = get_shot(guesses1)

# Check if player 1 hits a ship on player 2's board and update hit/miss records
    ships1, hit1, miss1, comp1,
    missed1 = check_shot(shot1, ships1, hit1, miss1, comp1)

# Shows status of player 2's board showing hits, misses, and destroyed ships
...
# Check if the computer's shot hits a ship and update records
    ships2, hit2, miss2, comp2,
    missed2 = check_shot(shot2, ships2, hit2, miss2, comp2)

# Show player 1's board with hits, misses, and destroyed ships
